...
func GetFilesByUploader(uploaderId int, startIdx int, num int) ([]*File, error) {
	var files []*File
	var err error
	err = DB.Where("uploader_id = ?", uploaderId).Order("id desc").Limit(num).Offset(startIdx).Find(&files).Error
	return files, err
}
