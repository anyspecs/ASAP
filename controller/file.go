...
func UploadFile(c *gin.Context) {
	...
	if currentUserRole < common.FileUploadPermission {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "无权上传文件！",
		})
...
func DeleteFile(c *gin.Context) {
	...
	err = model.DB.Where("id = ?", fileId).First(&fileObj).Error
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "文件不存在！",
	...
	// 权限验证：只有文件上传者或管理员才能删除文件
	if currentUserRole < common.RoleAdminUser && fileObj.UploaderId != currentUserId {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "无权删除此文件！只有文件上传者或管理员才能删除文件。",
		})
		return
	}
